...
# print(locations[1].get("exits").keys())
# print(locations[1])
...
while True:
    availableExits = ", ".join(locations[loc].get("exits").keys())  
           # you can also use 'locations[loc]["exits"].keys()' and '[][]' it means that we are going into the sub dictionary

 
    print(locations[loc].get("desc")) 
    #similarly for here 'locations[loc]["desc"]'
